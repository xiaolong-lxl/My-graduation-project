...
//更改状态
        function gaipai(data) {
            ...
            html +=
                ...
                '<div class="form-group">' +
                '      <div class="col-sm-offset-2 col-sm-10">' +
                '           <button type="submit" name="' + data.orderId + '" id="btnSubmit" class="btn btn-primary ">提交申请</button>' +
                '      </div>' +
                '</div>';
...
//提交更改值
        $("#gaipaiForm").on('click', '#btnSubmit', function () {
            var orderId = $(this).attr("name");
            var gaipaiTitle = $("#title").val();
            var gaipaiWhy = $("#gaipaiWhy").val();
            alert(orderId+"---"+gaipaiTitle+"---"+gaipaiWhy);
            $.ajax({
                url:"/tbGaipai/addGaiPai",
                data:{
                    leaderId: leaderId,
                    orderId:orderId,
                    gaipaiTitle:gaipaiTitle,
                    gaipaiWhy:gaipaiWhy
                },
                type:'POST',
                success:function (data) {
                    //alert("===success====="+data.isDeleted)
                    if (data.isDeleted) {
                        location.replace("/gaipaipage?leaderId="+data.leaderId);
                    }
                },
