...
//展示数据模块
        function show(data) {
            ...
            for (var i = 0; i < data.list.length; i++) {
                html +=
                    '<tr>' +
                    '<td>' + data.list[i].gaipaiId + '</td>' +
                    '<td>' + data.list[i].gaipaiTitle + '</td>' +
                    '<td id="tdway">' + data.list[i].gaipaiWhy + '</td>' +
                    '<td>' + new Date(data.list[i].gaipaiDate).format("yyyy-MM-dd") + '</td>';

                if (data.list[i].gaipaiStatu == "未查看") {
                    html += '<td class="danger">' + data.list[i].gaipaiStatu + '</td>';
                }
                if (data.list[i].gaipaiStatu == "已查看") {
                    html += '<td class="success">' + data.list[i].gaipaiStatu + '</td>';
                }
                html += '<td>' +
                    '<button class="btn btn-primary" name="' + data.list[i].gaipaiId + '" value="' + data.list[i].gaipaiId + '">取消申请' + '</button>&nbsp;' +
                    '</td>' +
                    '</tr>'
            }
            $("#tbody1").html(html);
...
//遍历一共有几页数据
        ...
        $("#tbody1").on('click','.btn-primary',function () {
            var gaipaiId = $(this).val();
            //alert(1111111111111+"====="+gaipaiId)
            if (confirm("确定要取消请假吗?")) {
                $.ajax({
                    url: "/tbGaipai/delGaiPai",
                    data: {
                        gaipaiId: gaipaiId
                    },
                    type: 'POST',
